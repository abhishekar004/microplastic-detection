...
MODEL_URL = os.getenv("MODEL_URL")  # optional URL to download weights if missing
MAX_MODEL_SIZE_MB = int(os.getenv("MAX_MODEL_SIZE_MB", "500"))  # default 500MB max
...
def get_or_load_model():
    ...
    model_loading = True
    try:
        # If model weights are missing, attempt to download if a MODEL_URL is provided
        if not os.path.exists(WEIGHTS_PATH):
            if MODEL_URL:
                logger.info(f"Model weights not found at {WEIGHTS_PATH}. Attempting download from {MODEL_URL}")
                try:
                    download_model(MODEL_URL, WEIGHTS_PATH, MAX_MODEL_SIZE_MB)
                except Exception as e:
                    raise FileNotFoundError(f"Model weights not found and download failed: {e}")
            else:
...
def download_model(url: str, dest_path: str, max_size_mb: int = 500):
    """Download model weights to the destination path, streaming and checking size.
    ...
    Raises:
        Exception: If download fails, exceeds max size, or status code != 200
    """
                ...
                if chunk:
                    ...
                    if total > max_bytes:
                        ...
                        try:
                            ...
                            pass
                        raise Exception(f"Downloaded file exceeds maximum allowed size of {max_size_mb} MB")
                    f.write(chunk)

        # Move to final path
        os.replace(tmp_path, dest_path)
        logger.info(f"Downloaded model to {dest_path} ({total} bytes)")
